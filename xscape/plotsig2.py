...
def plotsig(CVlist, randomTrialsCVlist,
            switchMin, switchMax, lossMin, lossMax, steps, outfile, outtext,
    ...
    # statistics
    greenStat = (100.0 * greenCounter / totalSamples)
    yellowStat = (100.0 * yellowCounter / totalSamples)
    redStat = (100.0 * redCounter / totalSamples)
    outputText = open(outtext, 'a')
    outputText.write("Using "+ str(numTrials)+ " trials \n")
    outputText.write("  Percentage green (p-value < 0.01):"+ str(greenStat)+ "\n")
    outputText.write("  Percentage yellow (0.01 <= p-value < 0.05):" + str(yellowStat)+"\n")
    outputText.write("  Percentage red (0.05 <= p-value):" + str(redStat) + "\n")
    outputText.close()
