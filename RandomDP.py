        ...
        for eh in postorder(hostTree, "hTop"):
            ...
            if vhIsATip:
                if vpIsATip and phi[vp] == vh:
                    A[(ep, eh)] = 0
                    # Contemporary event to be added to Dictionary
            ...
            #   to Dictionary using BestSwitchLocations
            if not vpIsATip:
                switchList = []
                ...
                if (C[(ep1, eh)] + BestSwitch[(ep2, eh)])<(C[(ep2, eh)] + \
                    ...
                    for item in BestSwitchLocations[(pChild2,vh)]:
                        ...
                        switchList.append(["T", (pChild1, vh), (pChild2, \
                            item[1]), (Score[(pChild1, vh)]*\
                            Score[(pChild2, item[1])])])
                    ...
                    for item in BestSwitchLocations[(pChild1,vh)]:
                        ...
                        switchList.append(["T", (pChild2, vh), \
                            (pChild1, item[1]), (Score[(pChild2, vh)]*\
                                Score[(pChild1, item[1])])])
                else: 
                    for item in BestSwitchLocations[(pChild2, vh)]:
                        if item[1] != None:
                            switchList.append(["T", (pChild1, vh), \
                                (pChild2, item[1]), (Score[(pChild1, vh)]*\
                                    Score[(pChild2, item[1])])])
                        else:
                            switchList.append(["T", (pChild1, vh), \
                                (pChild2, item[1]), Infinity])
                    for item in BestSwitchLocations[(pChild1,vh)]:
                        if item[1] != None:
                            switchList.append(["T", (pChild2, vh), \
                                (pChild1, item[1]), (Score[(pChild2, vh)]*\
...
def orderDTLRoots(DTL, vertex, level):
    """this function takes a DTL graph, one node, vertex, of the DTL graph, a 
    level, and a dictionary markingDict, and returns a list, keysL, that 
    ...
    keysL = []
    for i in range(len(DTL[vertex]) - 1):  #loop through each event in key
        event = DTL[vertex][i]
        child1 = event[1]
        child2 = event[2]
...
def preorderDTLsort(DTL, ParasiteRoot):
    ...
    list, orderedKeysL, that is ordered by level from largest to smallest, 
...
def preorderCheck(preOrderList):
    """This takes a list from preorderDTLsort and removes the duplicate 
    tuples"""
    newList = []
    preDict = {}
...
def addScores(treeMin, DTLDict, ParentsDict, ScoreDict, newDTL):
    """Takes the list of reconciliation roots, the DTL , a dictionary of 
    parent nodes, and a dictionary of score values, and returns the DTL with
    the normalized frequency scores calculated."""
