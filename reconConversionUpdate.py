...
def frequencyDict(DTL, reconciliation): #should this be scoreDict?
	"""This function takes as input the DTL, and the reconciliation and returns a dictionary, freqDict, that 
	has the same keys as the reconciliation and the values are the scores associated with each event"""
	freqDict = {}
	for key in reconciliation:
		events = DTL[key][:-1]
		for event in events:
